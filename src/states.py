...
class FollowWaypoints(smach.State):
    ...
    def execute(self, userdata):
        try:
            ...
            for i in range(len(userdata.waypoints)):
                point = Affine3()
                point.translation = userdata.waypoints[i].frame.translation
                point.quaternion = userdata.waypoints[i].frame.quaternion
                time = userdata.waypoints[i].time
                task.setPoseTarget(point, time)
                task.waitReachCompleted(
                    time
                )  # blocks till action is completed (or timeout has passed)
